...
export const Row = ({ index, data }) => {
    ...
    return (
        ...
        data && <tr key={index}>
            ...
            <td className='p-2 border-2 border-gray-200 dark:border-gray-600 font-semibold italic'>
                <a target="_blank" href={`https://etherscan.io/tx/${data.hash}`} className='underline text-sky-800 dark:text-sky-300'>
                    { data.hash && data.hash.slice(0,6) }...{ data.hash && data.hash.slice(-7) }
                </a>
            </td>
            <td className='p-2 border-2 border-gray-200 dark:border-gray-600 font-semibold italic'>
                <a target="_blank" href={`https://etherscan.io/address/${data.from}`} className='underline text-sky-800 dark:text-sky-300'>
                    { data.from && data.from.slice(0,6) }...{ data.from && data.from.slice(-7) }
                </a>
            </td>
            <td className='p-2 border-2 border-gray-200 dark:border-gray-600 font-semibold italic'>
                <a target="_blank" href={`https://etherscan.io/address/${data.to}`} className='underline text-sky-800 dark:text-sky-300'>
